...
object AppDestinations {
    ...
    const val CREATE_ACCOUNT_SCREEN_ROUTE = "create_account_screen"
    const val HOME_SCREEN_ROUTE = "home_screen"
    const val VIDEO_SCREEN_ROUTE = "video_screen"
    const val START_POSITION_SETTING_ROUTE = "start_position_setting_screen"
    
    // ★★★ ここから修正 ★★★
    // ルート定義を一つに統一し、常にオプショナルな menuId 引数を扱えるようにする
    const val CREATE_PRACTICE_MENU_ROUTE = "create_practice_menu?menuId={menuId}"
    // ★★★ ここまで修正 ★★★
    
    const val VIDEO_TRIM_SETUP_ROUTE = "video_trim_setup_screen"
    const val PRACTICE_LIST_SCREEN_ROUTE = "practice_list_screen"
    const val MENU_COMMENTS_ROUTE = "menu_comments" 
    const val MENU_COMMENTS_WITH_ARG_ROUTE = "menu_comments/{menuId}"
